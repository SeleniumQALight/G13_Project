...
public class LoginTestAllOneClass {
    ...
    public void invalidLogin() {
        ...
        logger.info("qaauto111 was entered in input Username");

        WebElement inputPassword = webDriver.findElement(
                By.xpath("//input[@placeholder='Password']"));
        inputPassword.clear();
        inputPassword.sendKeys("123456qwerty111");
        logger.info("123456qwerty111 was entered in input Password");

        webDriver.findElement(By.xpath("//button[text()='Sign In']")).click();
        logger.info("Button SignIn was clicked");
        Assert.assertTrue("User is LoggedIn: button SignIn is not visible",isButtonSighInIsVisible());
        Assert.assertFalse("User is LoggedIn: button SignOut is visible"
                , isButtonSighOutIsVisible());
        Assert.assertTrue("Invalid credential message is visible",invalidCredentialsMessageIsVisible());
    ...
    private boolean isButtonSighInIsVisible(){
        try{
    ...
    private boolean invalidCredentialsMessageIsVisible() {
        try {
