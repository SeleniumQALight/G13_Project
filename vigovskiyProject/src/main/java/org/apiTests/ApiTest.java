...
public class ApiTest extends BaseTestApi {
    ...
    public void getAllPostsByUser() {
        PostsDto[] actualResponse = given()
                ...
                //method#2 DTO or POJO опис джава об'екту
                .extract().body().as(PostsDto[].class);

        logger.info("full response body = " + Arrays.toString(actualResponse));
        ...
        for (int i = 0; i < actualResponse.length; i++) {
            Assert.assertEquals("UserName is not expexted in post " + i,
                    sharedUserName,
